...
- The demo depends on the **published** `timeslottr` package (`^1.0.0`), not the local source, so it builds standalone on Vercel (a `file:..` link can't resolve there, since root `dist/` is gitignored). The Playground only uses released API; bump the version in `demo/package.json` after a library release to surface new APIs in examples.
